...
### Support for WildFly variants

WildFly server offers different variants: `WildFly Preview` and `WildFly EE-10 (starting with WildFly 40)`.
You can configure the plugin to select the server variant in use.

#### Support for WildFly EE-10 variant

If you are targeting deployment to the WildFly EE-10 server, evolve the `discover-provisioning-info`:
...
#### Support for WildFly Preview variant
