...
public class ExpectedFileOneFileNoWildcardsTestCase extends AbstractLayerMetaDataTestCase {
    @Test
    public void testMatchesSingleFile() {
        Path p = createArchiveBuilder(ArchiveType.JAR)
                .addFile("test-one-no-wildcard.txt", "")
                .build();
        checkLayersForArchive(p, "expected-file-one-file-no-wildcard");
    }

    @Test
    public void testNoMatchSingleFile() {
        Path p = createArchiveBuilder(ArchiveType.JAR)
                .addFile("test-one-no-wildcardX.txt", "")
                .build();
